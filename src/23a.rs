...
fn main() {
    ...
    let ma_x = elves_pos.iter().map(|x| x.0).max().unwrap();
    let ma_y = elves_pos.iter().map(|x| x.1).max().unwrap();
    let mi_x = elves_pos.iter().map(|x| x.0).min().unwrap();
    let mi_y = elves_pos.iter().map(|x| x.1).min().unwrap();
    let ans = (ma_x - mi_x + 1) * (ma_y - mi_y + 1) - n as i32;
    println!("{}", ans);
}
