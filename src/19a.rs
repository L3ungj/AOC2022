...
fn main() {
    ...
    'input_loop: loop {
        ...
        let mut dp: Vec<Vec<Vec<Vec<Vec<Vec<item>>>>>> =
            vec![vec![vec![vec![vec![vec![(-1, -1, -1, -1); MAX_BRANCHES]; 11]; 11]; 11]; 11]; 25];
            ...
            for clay_robot in 0..11 {
                for obsidian_robot in 0..11 {
                    for geode_robot in 0..11 {
                        if my_cmp(
                            &ans,
                            &dp[24][ore_robot][clay_robot][obsidian_robot][geode_robot][0],
                        ) == Ordering::Less
                        {
                            ans = dp[24][ore_robot][clay_robot][obsidian_robot][geode_robot][0];
                        }
                    }
                }
            }
